...
# PYCOIN_GRS_BTC_PROVIDERS="blockchain.info blockexplorer.com blockcypher.com chain.so"
# PYCOIN_GRS_BTC_PROVIDERS="insight:http(s?)://hostname/url bitcoinrpc://user:passwd@hostname:8333"
...
def message_about_tx_cache_env():
    if main_cache_dir() is None:
        return "consider setting environment variable PYCOIN_GRS_CACHE_DIR=~/.pycoin_grs_cache to"\
...
def all_providers_message(method, netcode):
    if len(service_provider_methods(method, get_default_providers_for_netcode(netcode))) == 0:
        return "no service providers found for %s; consider setting environment variable "\
            "PYCOIN_GRS_%s_PROVIDERS" % (method, netcode)
